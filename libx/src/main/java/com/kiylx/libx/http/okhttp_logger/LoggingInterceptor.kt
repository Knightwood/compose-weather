 ...
 *  .log(VERBOSE)
 *  .singleTag(true)//true：在日志开头打印tag，false：每一行都打印tag
 *  .tag("tty1-HttpLogger")//设置tag
 *  .build()
 *  OkHttpClient().newBuilder()
 *      .addInterceptor(loggerInterceptor)//添加日志打印拦截器
 *      .build()
 */
class LoggingInterceptor private constructor(private val builder: Builder) : Interceptor {

    override fun intercept(chain: Interceptor.Chain): Response {
        val request = addQueryAndHeaders(chain.request())//添加自定义的header
    ...
    private fun proceedResponse(chain: Interceptor.Chain, request: Request): Response {
        return if (builder.isMockEnabled && builder.listener != null) {
            ...
            Response.Builder()
                .body(builder.listener!!.getJsonResponse(request)
    ...
    class Builder {
        ...
        var singleTag = false//是否指在开头打印一次tag
        ...
        fun singleTag(b: Boolean): Builder {
            singleTag = b;
            return this
        }
         ...
         * @see Logger
         */
        @Deprecated(message = "Create your own Logcat filter for best result",
            level = DeprecationLevel.ERROR)
